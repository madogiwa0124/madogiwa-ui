...
- Icons should include appropriate semantic meaning through context
        `}}}},i={render:e=>{let t=document.createElement(`div`),n=document.createElement(`div`);n.classList.add(`m-alert`),n.setAttribute(`role`,`alert`);let r=e.content,i=e.variant;return i!==`default`&&n.classList.add(`--${i}`),n.innerHTML=`
      <div class="m-alert__content">${r}</div>
    `,t.append(n),t},args:{content:`This is a basic alert message providing important information to the user.`,variant:`default`},play:async({canvasElement:e})=>{let r=n(e).getByRole(`alert`);await t(r).toBeInTheDocument(),await t(r).toHaveClass(`m-alert`);let i=r.querySelector(`.m-alert__content`);await t(i).toBeInTheDocument(),await t(i).toHaveTextContent(/alert message/)}},a={render:()=>{let e=document.createElement(`div`);e.style.display=`flex`,e.style.flexDirection=`column`,e.style.gap=`1rem`;for(let t of[{name:`default`,class:``,message:`This is a default alert for general information.`},{name:`primary`,class:`--primary`,message:`This is a primary alert for important actions.`},{name:`secondary`,class:`--secondary`,message:`This is a secondary alert for supporting information.`},{name:`tertiary`,class:`--tertiary`,message:`This is a tertiary alert for subtle notifications.`},{name:`danger`,class:`--danger`,message:`This is a danger alert for errors and critical issues.`},{name:`warning`,class:`--warning`,message:`This is a warning alert for caution messages.`}]){let n=document.createElement(`div`);n.setAttribute(`role`,`alert`),n.classList.add(`m-alert`),t.class&&n.classList.add(t.class),n.innerHTML=`<div class="m-alert__content">${t.message}</div>`,e.append(n)}return e},play:async({canvasElement:e})=>{let r=n(e).getAllByRole(`alert`);await t(r).toHaveLength(6);for(let[e,n]of[`default`,`primary`,`secondary`,`tertiary`,`danger`,`warning`].entries()){let i=r[e];if(i){await t(i).toBeInTheDocument(),await t(i).toHaveClass(`m-alert`),n!==`default`&&await t(i).toHaveClass(`--${n}`);let e=i.querySelector(`.m-alert__content`);await t(e).toBeInTheDocument(),await t(e).toHaveTextContent(new RegExp(n))}}},parameters:{docs:{description:{story:`Showcase of all available alert variants with their respective styling and use cases.`}}}},o={render:e=>{let t=document.createElement(`div`),n=document.createElement(`div`);n.setAttribute(`role`,`alert`),n.classList.add(`m-alert`);let r=e.content,i=e.icon,a=e.variant;return a!==`default`&&n.classList.add(`--${a}`),n.innerHTML=`
      <div class="m-alert__icons"><span class="icon">${i}</span></div>
      <div class="m-alert__content">${r}</div>
    `,t.append(n),t},args:{content:`This alert includes an icon to provide visual context and improve recognition.`,icon:`ℹ️`,variant:`primary`},play:async({canvasElement:e})=>{let r=n(e).getByRole(`alert`);await t(r).toBeInTheDocument(),await t(r).toHaveClass(`m-alert`),await t(r.querySelector(`.m-alert__icons`)).toBeInTheDocument();let i=r.querySelector(`.m-alert__content`);await t(i).toBeInTheDocument(),await t(i).toHaveTextContent(/visual context/)}},s={render:e=>{let t=document.createElement(`div`),n=document.createElement(`div`);n.setAttribute(`role`,`alert`),n.classList.add(`m-alert`);let r=e.content,i=e.icon,a=e.variant;a!==`default`&&n.classList.add(`--${a}`),n.innerHTML=`
      <div class="m-alert__icons">${i}</div>
      <div class="m-alert__content">${r}</div>
    `;let o=document.createElement(`div`);o.classList.add(`m-alert__actions`);let s=document.createElement(`button`);return s.classList.add(`m-btn`,`--icon-only`,`--rounded`),s.style.backgroundColor=`transparent`,s.innerHTML=`✕`,s.setAttribute(`aria-label`,`Close alert`),s.addEventListener(`click`,()=>{n.remove()}),o.append(s),n.append(o),t.append(n),t},args:{content:`This alert includes action buttons that allow users to interact with or dismiss the message.`,icon:`⚠️`,variant:`warning`},play:async({canvasElement:e})=>{let r=n(e),i=r.getByRole(`alert`);await t(i).toBeInTheDocument(),await t(i).toHaveClass(`m-alert`),await t(i.querySelector(`.m-alert__icons`)).toBeInTheDocument(),await t(i.querySelector(`.m-alert__content`)).toBeInTheDocument();let a=r.getByRole(`button`);await t(a).toBeInTheDocument(),await t(a).toHaveClass(`m-btn`);let o=i.querySelector(`.m-alert__actions`);await t(o).toBeInTheDocument(),await t(o).toContainElement(a)}},c={render:e=>{let t=document.createElement(`div`),n=document.createElement(`div`);n.setAttribute(`role`,`alert`),n.classList.add(`m-alert`);let r=e.content,i=e.variant,a=e.autoDismiss,o=e.autoDismissProgress;return i!==`default`&&n.classList.add(`--${i}`),a&&a!==`0s`&&(n.dataset.autoDismiss=a,o&&n.classList.add(`--with-progress`)),n.innerHTML=`
      <div class="m-alert__content">${r}</div>
    `,t.append(n),t},args:{content:`This alert will automatically dismiss after the specified duration. Watch the progress bar!`,variant:`primary`,autoDismiss:`3s`,autoDismissProgress:!0},play:async({canvasElement:e})=>{let r=n(e).getByRole(`alert`);await t(r).toBeVisible(),await t(r).toHaveClass(`m-alert`);let i=r.querySelector(`.m-alert__content`);await t(i).toBeInTheDocument(),await t(i).toHaveTextContent(/automatically dismiss/),r.classList.contains(`--with-progress`)&&await t(r).toHaveClass(`--with-progress`)},parameters:{docs:{description:{story:`Alert with auto-dismiss functionality and optional progress bar. The alert will fade out after the specified duration.`}}}},l={render:()=>{let e=document.createElement(`div`);e.style.display=`flex`,e.style.flexDirection=`column`,e.style.gap=`1rem`;for(let t of[{variant:`primary`,icon:`✅`,content:`Successfully saved your changes!`},{variant:`warning`,icon:`⚠️`,content:`Please review the information before continuing.`},{variant:`danger`,icon:`❌`,content:`An error occurred while processing your request.`}]){let n=document.createElement(`div`);n.setAttribute(`role`,`alert`),n.classList.add(`m-alert`,`--${t.variant}`),n.innerHTML=`
        <div class="m-alert__icons">${t.icon}</div>
        <div class="m-alert__content">${t.content}</div>
      `,e.append(n)}return e},parameters:{docs:{description:{story:`Multiple alerts can be stacked to show different types of messages simultaneously.`}}}},i.parameters={...i.parameters,docs:{...i.parameters?.docs,source:{originalSource:`{
  render: args => {
...
}`,...l.parameters?.docs?.source}}},u=[`Default`,`AllVariants`,`WithIcon`,`WithActions`,`AutoDismiss`,`MultipleAlerts`]}))();export{a as AllVariants,c as AutoDismiss,i as Default,l as MultipleAlerts,s as WithActions,o as WithIcon,u as __namedExportsOrder,r as default};